// Direct test data creation for Client 3 using raw SQL
...
async function createTestData() {
    ...
    try {
        ...
        const roadmapId = roadmapResult[0]?.id || (await sql`SELECT id FROM roadmaps WHERE client_id = 3 AND title = 'ISO 27001:2022 Compliance Roadmap'`)[0].id;
        ...
        const planId = planResult[0]?.id || (await sql`SELECT id FROM implementation_plans WHERE client_id = 3 AND roadmap_id = ${roadmapId}`)[0].id;
        ...
        for (const [pId, title, desc, status, priority, assigned, effort, cost, start, due] of tasks) {
            await sql`
                ...
                ) VALUES (${pId}, ${title}, ${desc}, ${status}, ${priority}, ${assigned}, ${effort}, ${cost}, ${start}, ${due})
        ...
        for (const [clientId, title, desc, inherent, residual, likelihood, impact, status, treatment] of risks) {
            await sql`
                ...
                ) VALUES (${clientId}, ${title}, ${desc}, ${inherent}, ${residual}, ${likelihood}, ${impact}, ${status}, ${treatment})
