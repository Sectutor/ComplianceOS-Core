...
import { pgTable, integer, varchar, text, timestamp, boolean, json, jsonb, serial, index, uniqueIndex } from "drizzle-orm/pg-core";
...
export const licenseActivations = pgTable("license_activations", {
  ...
  return {
    ...
    uniqueClientLicense: uniqueIndex("uniq_license_activations_client").on(
      table.clientId,
      table.licenseStatus
    ).where(eq(table.licenseStatus, 'active')),
  };
});
...
export const licenseValidationLogs = pgTable("license_validation_logs", {
  ...
  validationResult: jsonb("validation_result").$type<Record<string, any>>(),
  errorMessage: text("error_message"),
  missingFeatures: jsonb("missing_features").$type<string[]>(),
